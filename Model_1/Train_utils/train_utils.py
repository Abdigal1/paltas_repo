...
def train_test(model,optimizer,train_set,test_set,batch_size,use_cuda,loss_function,epochs,data_train_dir,in_device=None,checkpoint_epoch=0):
    ...
    for epoch in tqdm(range(checkpoint_epoch,epochs),desc="Epoch"):
        ...
        dataloader_train=torch.utils.data.DataLoader(train_set,batch_size=batch_size,shuffle=True,num_workers=12,drop_last=drop,persistent_workers=True)
        dataloader_test=torch.utils.data.DataLoader(test_set,batch_size=batch_size,shuffle=True,num_workers=12,drop_last=drop,persistent_workers=True)
